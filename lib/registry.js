...
export class RoleRegistry {
  ...
  load() {
    ...
    for (const [role, sessionId] of Object.entries(parsed)) {
      ...
      const prevRole = this.bySession.get(sessionId);
      if (prevRole !== undefined && prevRole !== role) this.byRole.delete(prevRole);
      this.byRole.set(role, sessionId);
      this.bySession.set(sessionId, role);
    }
  }
  ...
  bind(role, sessionId) {
    ...
    const prevSession = this.byRole.get(role);
    if (prevSession !== undefined && prevSession !== sessionId) {
      this.bySession.delete(prevSession);
    }
    this.byRole.set(role, sessionId);
    this.bySession.set(sessionId, role);
    this.save();
